   ...
   if($result){
    ...
    if (isset($_POST['send_message'])){
      if(isset($_POST['message']) && !empty($_POST['message'])){
          ...
          $query = "INSERT INTO `conversations` (`cid`, `pid`, `did`, `pm`, `dm`, `time`) VALUES (NULL, '{$pid}', '".$_SESSION['id']."', '', '$message',  CURRENT_TIMESTAMP)";
...
if(mysqli_num_rows($result) == 1){
    ...
    <!-- 2nd Column -->
    <div class="col-12 col-sm-6 mt-3 mt-sm-0 col2"> 
    <div class="row">
    <div class="row-header"> <h5><?php echo $drow['firstname']." ".$drow['lastname'];  ?></h5></div>
    </div>
     
    <div class="row">
       <div class="col-2 mt-2">
         <div class=""><a href="doc_conversations.php" class="btn btn-sm btn-success">Back &#8656;</a></div>
         <div class="mt-2"><a href="doc_view_patient.php?pid=<?php echo $pid ?>" class="btn btn-sm btn-success">View Profile</a></div>
       </div>                                             
...
<form method="POST" action="doc_chatscreen.php" class="needs-validation" novalidate>
...
<div class="row msgrow">
   ...
   <?php if (isset($successmsg)){echo "<div class='success'>".$successmsg."</div>";} ?> 
     <div class="container">
<?php 
  $fquery = "SELECT * FROM `med_files` WHERE `pid` = '{$pid}'";
  $fresult = mysqli_query($conn, $fquery);
 if($fresult){
    while ($frow = mysqli_fetch_assoc($fresult)){
        
           echo "<div class='alert alert-success mt-2' role='alert'><a href='med_files/".$frow['file']."'><p>".$frow['file']."</p></a></div>";
...
function getChat(){
  var xmlhttp = new XMLHttpRequest();
	...
	xmlhttp.open('GET', 'chats.inc.php?pid=<?php echo $pid ?>', true);
	xmlhttp.send();
