...
if(!isset($_SESSION["admin_log"]) || $_SESSION["admin_log"] !== true){
 ...
 if($image_url == "nopic"){
  ...
  $img_src = "uploads/".$image_url;
    ...
    //search
    if (isset($_POST['key_username']) && !empty($_POST['key_username'])){
          $keyword = $_POST['key_username'];
        if(strlen($keyword) >= 1){
             $search_query = "SELECT * FROM feedback where `username` like '%$keyword%'";   
    ...
    if(strlen($keyword) >= 1){
         $search_query = "SELECT * FROM feedback where `date` like '%$keyword%'";   
   ...
   <div class="col-5">
    <h6> Search by Username</h6>
        <form action="" method="POST" class="w-100 needs-validation" novalidate>
	       <input type="text" name="key_username" class="form-control" placeholder="Enter Username" required> 
...
<div class="card border-success mb-3 mt-4" >
  <div class="card-header bg-transparent border-success">
     <span style="font-weight:bold;">By: <?php echo $row['username']; ?></span>
