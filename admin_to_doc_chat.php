  ...
  $img_src = "uploads/".$image_url;
   ...
   if (isset($_POST['send_message'])){
      if(isset($_POST['message']) && !empty($_POST['message'])){
          ...
          $query = "INSERT INTO `admin_to_doc` (`cid`, `aid`, `did`, `a_msg`, `d_msg`, `date`) VALUES (NULL, '{$id}', '{$did}', '$message' ,'',  CURRENT_TIMESTAMP)";
...
if(mysqli_num_rows($result) == 1){
    $drow = mysqli_fetch_assoc($result);
     ...
     ?>
    <!-- 2nd Column -->
    <div class="col-10 mx-auto mt-2 mt-sm-0 col-sm-10 col2">
    <div class="row">
    ...
    <div class="row">
       <div class="col-4 mt-2">
         <div class=""><a href="admin_to_doc.php" class="btn btn-sm btn-success">Back &#8656;</a></div>
       </div>                                             
...
<?php 
  $cquery = "SELECT * FROM `admin_to_doc` WHERE `aid` = '".$_SESSION['id']."' and did={$did}";
  $cresult = mysqli_query($conn, $cquery);
 if($cresult){
    while ($crow = mysqli_fetch_assoc($cresult)){
        
       if ($crow['a_msg'] !=""){  echo "<div class='alert alert-primary mt-2 am' role='alert'><p>".$crow['a_msg']."</p></div>";


       } 

       if ($crow['d_msg'] !=""){ echo "<div class='alert alert-success mt-2 dm' role='alert'><p>".$crow['d_msg']."</p></div>";
    } 
...
<form method="POST" action="admin_to_doc_chat.php" class="needs-validation" novalidate>
